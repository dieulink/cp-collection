...
point_list = rand_list(n, 1, Xmax, True)

for i in range(0, n):
    candidate_id = ""
    flag = True
    while flag:
        candidate_id = rand_str(rand_int(1, Lmax), letters)
        if candidate_id not in id_dict:
            id_dict[candidate_id] = 1
            flag = False
        else:
            flag = True
    print(point_list[i], candidate_id)
